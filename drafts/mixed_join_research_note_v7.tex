% Versioned draft v7, 29 August 2026.
% Initial-submission candidate prepared for Discussiones Mathematicae Graph
% Theory (DMGT). The frozen v3--v6 sources are unchanged. This version adds
% the verified public repository location to the v6 reproducibility and
% exposition revision. It does not claim human peer review.
...
according to whether they meet the designated complete factor
\(C=V(K_r)\). The \(C\)-meeting
branch is governed by partitions of \(T\) into two induced cliques,
while the \(C\)-avoiding branch is governed by a tree parameter
\(\beta(T)\), where
...
We characterize every \(\beta\)-feasible set by vertex degrees and the
number of selected neighbors, and give a linear-time, linear-space
...
verification routes compare the dynamic program with exhaustive search over
the defining local constraints and compare the theorem with a definition-first
shortest-path checker. These calculations are supporting evidence, not proof.
\end{abstract}
...
families, and Section~\ref{sec:computation} records two bounded verification
routes and their different logical roles. The computations are supporting
evidence rather than proof. The precise comparison with the closest product,
join, and fan results is given above; no claim about graph classes outside the
stated scope is needed for the results proved below.
...
set meeting the designated complete factor exists precisely when the tree can be
partitioned into two induced cliques, and the maximum in that branch is
\(r+q_2(T)\). A set avoiding the designated complete factor is dual general
