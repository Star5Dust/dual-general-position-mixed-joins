% Versioned draft v3, 29 August 2026.
% Based on v2 (the externally compiled D648DD44... source). This version
% changes layout only: it keeps two lead-ins with their displays/tables and
% narrows the first reproducibility table to remove its alignment overfull.
...
\begin{quote}
Internal working draft, 29 August 2026. This document makes no novelty
or priority claim. The literature cutoff and coverage limitations are
recorded in \path{notes/mixed_join_literature_positioning.md}.
\end{quote}
...
two conditions~\cite{tian-klavzar-variety}.
...
of~\cite{tian-et-al-removal}. The exact display in the version of record
has not been independently checked and remains \texttt{UNKNOWN}.

This note studies \(K_r+T\) for \(r\ge 1\) and for trees \(T\) of order
...
than proof. A bounded
literature audit did not find the full all-tree, arbitrary-\(r\) result;
because several subscription databases were not directly checked and
terminology may differ, its global novelty remains \texttt{UNKNOWN}.

\section{Preliminaries}\label{sec:preliminaries}

All graphs in this note are finite and simple. For a graph \(G\), its
...
stricter parameter; this note makes no claim that the parameter itself
is new.
...
\cite{jiang-complete-first-factor} was available only as a preprint in the
bounded search completed on 29 August 2026; whether a peer-reviewed version
will appear remains \texttt{UNKNOWN}. The bounded computations in
Section~\ref{sec:computation} support the implementation but do not prove the theorem. The
literature audit did not directly cover MathSciNet, Scopus, or Web of
Science, and some open indexes were unavailable; therefore the global
novelty and priority of the all-tree, arbitrary-\(r\) mixed-join result
remain \texttt{UNKNOWN}. The exact formula display in the
version-of-record body of~\cite{tian-et-al-removal} also remains \texttt{UNKNOWN},
although its arXiv v2 display was checked.

Finally, this note treats only \(K_r+T\) with \(r\ge 1\) and
